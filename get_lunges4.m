function get_lunges4(directory, videoname, classifiername, excluded) %PDM
  %Ben Habermeyer
    ...
    % PDM 5-12-2020: loads well centers from calibrator instead of from
    % python
...
%     % debugging
    ...
    load([directory,'\',videoname,'_calibration.mat'],'calib')
    yvals = calib.centroids(:,1);
    xvals = calib.centroids(:,2);
    
%     % well locations in calib file are out of order
%     figure; title('unsorted'); hold on
%     set(gca,'YDir','reverse');
%     for w = 1:length(xvals)
%        plot(xvals(w),yvals(w),'.','markersize',12)
%        text(xvals(w),yvals(w),num2str(w),'fontsize',12,'fontweight','bold') 
%     end
    
    % sort by y position
    pairs = [xvals,yvals];
    pairs = sortrows(pairs,[2]);
    xvals = pairs(:,1); yvals = pairs(:,2);
    
%     figure; title('sorted by val only'); hold on;
%     set(gca,'YDir','reverse');
%     for w = 1:length(xvals)
%         plot(xvals(w),yvals(w),'.','markersize',12)
%         text(xvals(w),yvals(w),num2str(w),'fontsize',12,'fontweight','bold')
%     end
    ...
    % sort by x position
    pairs(1:4,1:2) = sort(pairs(1:4,1:2),1);
    pairs(5:8,1:2) = sort(pairs(5:8,1:2),1);
    pairs(9:12,1:2) = sort(pairs(9:12,1:2),1);
    xvals = pairs(:,1); yvals = pairs(:,2);
    
%     figure; title('fully sorted'); hold on;
%     set(gca,'YDir','reverse');
%     for w = 1:length(xvals)
%         plot(xvals(w),yvals(w),'.','markersize',12)
%         text(xvals(w),yvals(w),num2str(w),'fontsize',12,'fontweight','bold')
...
%     end
    ...
    for i = 1:12
       circle_centers(i,1) = xvals(i); %PDM
       circle_centers(i,2) = yvals(i); %PDM
    end
