...
class BehaviorClassifier(object):
    ...
    PDM changes & notes
    1. fixed indentation and changed paths to run on my computer
    2. created 'getLungesV4.m' to get well centers from the calibration.mat file instead of find_centers(), which was not working on the test vid
    3. modified checkbox_grid() to show an image of the first frame of the video
    4. added track_wings(), currently this just launches the APT GUI and waits for the user to finish
    TO DO: modify the JAABA parts to use the APT trk files, automate wing tracking
    '''
    ...
    def __init__(self):
        ...
        self.code_path = r'C:\Users\PDMcClanahan\Dropbox\Kayser lab\tracking and behavior\Behavior_Classifier_Ben'
        #classifier filename
        self.classifier = 'LungeV3.jab'
        #FlyTracker path on computer
        self.flytracker_path = r'C:\Users\PDMcClanahan\Dropbox\Kayser lab\tracking and behavior\Behavior_Classifier_Ben\FlyTracker-1.0.5'
        #JAABA path on computer
        self.jaaba_path = r'C:\Users\PDMcClanahan\Dropbox\Kayser lab\tracking and behavior\Behavior_Classifier_Ben\JAABA-master\perframe'

        #background variables
        self.num_wells = 12
        self.n_cpus = 2

        # #PDM inputs for debugging
        # self.calib = 'C:/Users/PDMcClanahan/Dropbox/Kayser lab/tracking and behavior/short_test_vid/MAH00479_cropped_cropped_calibration.mat'
        # self.classifier = 'LungeV3.jab'
        # self.code_path = 'C:\\Users\\PDMcClanahan\\Dropbox\\Kayser lab\\tracking and behavior\\Behavior_Classifier_Ben'
        # self.excluded_wells = [3, 4, 7, 8, 12]
        # self.filename = 'C:/Users/PDMcClanahan/Dropbox/Kayser lab/tracking and behavior/short_test_vid/MAH00479_cropped_cropped.MP4'
        # self.flytracker_path = 'C:\\Users\\PDMcClanahan\\Dropbox\\Kayser lab\\tracking and behavior\\Behavior_Classifier_Ben\\FlyTracker-1.0.5'
        # self.fullname = 'MAH00479_cropped_cropped.MP4'
        # self.jaaba_path = 'C:\\Users\\PDMcClanahan\\Dropbox\\Kayser lab\\tracking and behavior\\Behavior_Classifier_Ben\\JAABA-master\\perframe'
        # self.n_cpus = 2
        # self.name = 'MAH00479_cropped_cropped'
        # self.num_wells = 12
        # self.root = 'C:/Users/PDMcClanahan/Dropbox/Kayser lab/tracking and behavior/short_test_vid'
        ...
        print('passed calibrate_tracker') #PDM debugging
                
        
        self.checkbox_grid()
        # print('passed checkbox_grid') #PDM debugging
        
        # leave commented, this version uses centers found by FlyTracker in getLunges4.m
        # self.find_centers()
        # print('passed find_centers') #PDM debugging
        
        #track the video
        self.run_tracker()
        print('passed run_tracker') #PDM debugging
        
        # APT tracking of wings
        self.track_wings()
        print('passed track_wings')
        
        #JAABA stuff
        #reorganize the folders for JAABA
        self.prepare_JAABA()
        print('passed prepare_JABBA') #PDM debugging
        
        #run the JAABA program
        self.classify_behavior()
        print('passed classify_behavior') #PDM debugging
        
        #get the output
        self.get_lunge_data()
        print('passed get_lunge_data') #PDM debugging
    ...
    def crop_start(self):
        ...
        """
        print('At start of crop_start, self.filename is'+self.filename)
        ...
        print('self.fullname is '+self.fullname) #PDM
        self.name = self.name + '_cropped'
        print('self.name is '+self.name) #PDM
        #pdb.set_trace() #PDM debugging
        self.filename = self.root + '/' +self.fullname
        print('At end of crop_start, self.filename is'+self.filename)
    ...
    def calibrate_tracker(self):
        ...
        print('video is '+ video) #PDM debugging
        print('calibration is '+ self.calib) #PDM debugging
    ...
    def checkbox_grid(self):
        ...
        Label(master,image = ph, bg = "black", width = im_width).grid(row=0, columnspan = 4,  pady = 4)
    ...
    def track_wings(self):
        ...
        """
        print('Launching APT Wing Tracking')
        os.chdir(self.code_path)
        try:  # try quiting out of any lingering matlab engines
            eng.quit()
        except:  # if not just keep going with the code
        ...
        eng = matlab.engine.start_matlab()
        #eng = matlab.engine.start_matlab("-desktop") # use to start a visible instance of MATLAB
        
        #videoname and trx file needed for tracking
        videoname = self.root
        trxfile = self.root+'/trx.mat'
        #wing_alg_file = self.code_path+'\\wing_tracker4.lbl'
        eng.addpath(eng.genpath(r'C:\Users\PDMcClanahan\APT-develop'))
        eng.StartAPT()
        ...
        instructions = 'INSTRUCTIONS: \n1. File > Open > wing_tracker_v4.lbl/n2. Add Movie > (pick movie and associated trx file)\n3. (with new movie highlighted) Switch to Movie\n4. Track > Tracking Algorithm > CPR\n5. Track (wait for finish)\n6. File > Import/Export > Export Predictions to Trk File (Current Video) > (use default name) OK'    
    ...
    def prepare_JAABA(self):
        ...
        print('WARNING: need to uncomment next two lines!')
        # shutil.move(trx_path, destination) #PDM debugging
        # shutil.move(perframe_path, destination) #PDM debugging
    ...
    def get_lunge_data(self):
        ...
        eng.get_lunges4(directory, videoname, classifiername, excluded, nargout = 0)
        try:  # try quiting out of any lingering matlab engines
            eng.quit()
        except:  # if not just keep going with the code
            print('could not find any engines to quit')
